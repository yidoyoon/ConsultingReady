...
| 열어서 **보기만** 하고 닫기 | ❌ 손대지 않음 (단, PowerPoint 메모 창은 접어서 저장) |
| 작업 중 Ctrl+S 만 (닫지 않음) | ❌ 손대지 않음 (작업 중 방해 안 함) |
| 일시정지 중 | ❌ 아무 동작 안 함 |
| 파일명 필터에서 제외됨 | ❌ 손대지 않음 |
| **백업 실패** | ❌ 수정하지 않고 그대로 닫음 |

**딱지 검사는 예외**입니다. 파일을 읽기만 하므로 저장 여부와 관계없이 항상 검사하고, 발견되면 **저장하지 않고 알림만** 띄웁니다.

**메모 창 접기도 예외**입니다. 발표 직전 상태를 맞추는 것이 목적이므로, 사용자가 저장을 했는지와 관계없이 메모 창이 펼쳐져 있으면 접습니다.

| 상황 | 동작 |
|---|---|
| 열어서 보기만 한 PowerPoint | 메모 창을 접고 **저장**합니다 |
| 저장하지 않은 편집이 있는 PowerPoint | 메모 창만 접고 **저장은 하지 않습니다**. PowerPoint가 띄우는 "저장하시겠습니까?"에 사용자가 답한 대로 처리됩니다 |

> OneDrive / SharePoint 문서도 같게 동작합니다. 보기만 한 문서라도 메모 창이 펼쳐져 있었다면 접은 상태로 저장되므로 **버전 기록에 한 줄이 추가**됩니다. 부담스러우면 설정에서 **메모 창 접고 저장**을 끄시면 됩니다.
