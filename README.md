...
`main` 브랜치에 올라가는 모든 변경은 아래 검사를 통과해야 합니다.

| 검사 | 내용 |
|---|---|
| **CodeQL** | GitHub의 코드 분석 엔진으로 취약점·의심 패턴 탐지 |
| **Bandit** | Python 보안 린터 (위험한 함수 사용, 하드코딩된 비밀 등) |
| **pip-audit** | 사용하는 라이브러리에 알려진 취약점(CVE)이 있는지 확인 |
| **네트워크 코드 검사** | 소켓·HTTP 요청 등 외부 통신 코드가 들어갔는지 확인 |

검사 결과는 저장소의 **Actions** 탭과 **Security** 탭에서 볼 수 있습니다.
