...
### 수정 전 원본을 백업
- **백업 폴더** — `찾아보기...` 로 지정합니다. 기본값은 `%LOCALAPPDATA%\ConsultingReady\backup`
- **원본 1개당 보관 개수** — 기본 20개. 넘으면 오래된 것부터 지웁니다.

### 파일명 필터 (정규식)
- **제외** — 패턴과 일치하는 파일은 건드리지 않음
- **허용** — 패턴과 일치하는 파일만 처리
...
| 패턴 | 의미 |
|---|---|
| `최종\|배포` | 이름에 "최종" 또는 "배포"가 들어간 파일 |
| `^TARA` | 이름이 TARA로 시작하는 파일 |
| `\.xlsm$` | 매크로 문서(.xlsm) |

대소문자를 구분하지 않고, 파일명에 **부분 일치**합니다. 입력하는 즉시 정규식이 올바른지 검사하며, 잘못된 정규식은 저장이 막힙니다.
...
이 프로그램은 여러분의 업무 문서를 읽고 저장합니다. 그래서 **"숨겨진 악성 코드가 없다"** 는 것을 확인하실 수 있도록 아래 절차를 갖췄습니다.

### 배포 파일의 출처 증명 (Build Provenance)

Release에 올라간 `ConsultingReady.exe` 는 **제작자 PC에서 만들어 올린 것이 아니라, GitHub Actions가 이 저장소의 소스로 직접 빌드한 것**입니다. GitHub가 서명한 증명서가 함께 발행되므로 아래 명령으로 누구나 확인할 수 있습니다.

```bash
gh attestation verify ConsultingReady.exe --repo <OWNER>/ConsultingReady
```

이 명령이 통과하면 **그 exe가 정확히 이 저장소의 어떤 커밋에서 만들어졌는지** 확인됩니다. 즉 소스에 없는 코드가 exe에 몰래 들어갈 수 없습니다.

### 악성 행위 능력 검사

일반적인 보안 도구가 **취약점**(실수로 생긴 약점)을 찾는다면, 이 검사는 **악성 프로그램이 하는 일**(의도적으로 넣은 기능)을 찾습니다. 악성코드가 목적을 달성하려면 반드시 필요한 능력들이 코드에 있는지 항목별로 확인하고, 하나라도 발견되면 **빌드를 실패시킵니다**.

| 검사 항목 | 왜 보는가 |
|---|---|
| 네트워크 통신 | 정보 유출·추가 페이로드 다운로드 통로 |
| 외부 프로그램 실행 | 2차 페이로드 실행, 권한 상승 |
| 동적 코드 실행 | `eval`/`exec` 로 숨긴 코드 실행 |
| 난독화·인코딩된 페이로드 | 악성 코드를 문자열로 위장 |
| 자동 실행 등록(지속성) | 몰래 상주·재감염 |
| 입력 가로채기·화면 감시 | 키로거, 화면 캡쳐 |
| 자격증명 접근 | 저장된 비밀번호·토큰 탈취 |
| 광범위 파일 탐색·암호화 | 랜섬웨어, 대량 문서 수집 |
| 보안 기능 무력화 | 백신·방화벽·로그 끄기 |

검사기는 [`tools/capability_audit.py`](tools/capability_audit.py) 에 전부 공개돼 있습니다.

**검사기가 실제로 작동하는지도 함께 증명합니다.** 아무것도 탐지하지 못하는 검사는 통과해도 의미가 없기 때문에, CI 는 매번 **일부러 악성 패턴을 심은 샘플**을 검사기에 넣어 전 항목을 탐지하는지 확인합니다. 탐지에 실패하면 그 자체로 빌드가 실패합니다.

### 일반 취약점 검사

| 검사 | 내용 |
|---|---|
| **CodeQL** | GitHub 코드 분석 엔진 (`security-extended` 규칙) |
| **Semgrep** | 보안 규칙셋 정적 분석 |
| **Bandit** | Python 보안 린터 |
| **pip-audit** | 사용 라이브러리의 알려진 취약점(CVE) |
| **Gitleaks** | 하드코딩된 토큰·키 검사 (**전체 커밋 이력**) |

모든 검사는 `main` 에 올라가는 변경과 PR 마다, 그리고 **매주 월요일**에 자동 실행됩니다. 결과는 저장소의 **Actions** 탭 요약과 **Security** 탭에서 볼 수 있습니다.

### SHA256 체크섬

Release에 `SHA256SUMS.txt` 가 함께 올라갑니다. 내려받은 파일이 변조되지 않았는지 확인할 수 있습니다.

```powershell
Get-FileHash ConsultingReady.exe -Algorithm SHA256
```

### 이 프로그램이 하지 않는 일

- **네트워크 통신을 전혀 하지 않습니다.** 인터넷 연결이 필요 없고, 어떤 데이터도 외부로 보내지 않습니다.
...
**파일명 필터**를 **제외** 모드로 켜고 패턴을 넣으세요. 예: `템플릿|샘플`
